...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, load_iteration, skip_compose, ref_idx):
    ...
    if skip_compose==0:
            ...
            mask = imageio.imread(f'{dataset.source_path}/reference/mask/{ref_name}.png')


            depth_path_large = f'{dataset.source_path}/completed_depth_{ref_name}.npy'
            depth = torch.tensor(np.load(depth_path_large, allow_pickle=True)).cuda()

            c2w = reference_cam.world_view_transform.T.inverse()
            W, H = reference_cam.image_width , reference_cam.image_height
            ...
            ref_img = reference_cam.original_image

            mask_flat = mask.reshape(-1)
            masked_xyz = xyz[mask_flat == 1].type(torch.float32)
            masked_rgb = ref_img.reshape(3, -1).T[mask_flat == 1].type(torch.float32)
            ...
            gaussians.compute_3D_filter(cameras=trainCameras)
            scene.save(301)
            point_cloud_path = os.path.join(dataset.model_path, "point_cloud/iteration_{}".format(301))
            savepath = point_cloud_path
            np.savetxt(savepath + '/points.txt', np.array(shape), fmt='% 06d')
            print('save initial point')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip_compose", type=int, default = 0)
    parser.add_argument("--ref_idx", type=int, default = 186)
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, \
             args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.load_iteration, args.skip_compose, args.ref_idx)
