...
class GaussianModel:
    ...
    def add_points(self, points):
        ...
        xyzs = points[:,:3]
        f_dc = points[:,3:].unsqueeze(1)
        f_dc = RGB2SH(f_dc)
        # f_dc = torch.ones_like(f_dc)*(-100)
        dist2 = torch.clamp_min(distCUDA2(xyzs.detach().clone().float().cuda()), 0.0000001)
        scales = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)
        rots = torch.zeros((points.shape[0], 4), device="cuda")
        rots[:, 0] = 1
        features = torch.zeros((xyzs.shape[0], 3, 15)).float().cuda()
        # opacities = self.inverse_opacity_activation(10000 * torch.ones((points.shape[0], 1), dtype=torch.float, device="cuda"))
        opacities = inverse_sigmoid(1.0 * torch.ones((points.shape[0], 1), dtype=torch.float, device="cuda"))

        new_xyz = nn.Parameter(xyzs.requires_grad_(True))
        new_features_dc = nn.Parameter(f_dc.contiguous().requires_grad_(True))
        new_features_rest = nn.Parameter(features.transpose(1, 2).contiguous().requires_grad_(True))
    ...
    def stop_grad(self, training_args, no_grad_part_length):
        ...
        self.filter_3D = torch.nn.Parameter(torch.cat([set_requires_grad(self.filter_3D[:no_grad_part_length], False), set_requires_grad(self.filter_3D[no_grad_part_length:], True)]))
        ...
        # l = [
        #     {'params': [self._xyz], 'lr': training_args.position_lr_init * self.spatial_lr_scale, "name": "xyz"},
        #     {'params': [self.filter_3D], 'lr': training_args.is_masked_lr, "name": "filter_3D"},
        #     {'params': [self._features_dc], 'lr': training_args.feature_lr, "name": "f_dc"},
        #     {'params': [self._features_rest], 'lr': training_args.feature_lr / 20.0, "name": "f_rest"},
        #     {'params': [self._opacity], 'lr': training_args.opacity_lr, "name": "opacity"},
        #     {'params': [self._scaling], 'lr': training_args.scaling_lr, "name": "scaling"},
    ...
    def load_ply(self, path, spatial_lr_scale):
    ...
    def load_ply_norest(self, path, use_train_test_exp = False, cam_infos=None):
        plydata = PlyData.read(path)
        ...
        features_dc[:, 1, 0] = np.asarray(plydata.elements[0]["f_dc_1"])
        features_dc[:, 2, 0] = np.asarray(plydata.elements[0]["f_dc_2"])
        features_dc = np.clip(features_dc, -1.7, 1.7)
        extra_f_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("f_rest_")]
        extra_f_names = sorted(extra_f_names, key = lambda x: int(x.split('_')[-1]))
        assert len(extra_f_names)==3*(self.max_sh_degree + 1) ** 2 - 3
        features_extra = np.zeros((xyz.shape[0], len(extra_f_names)))
        for idx, attr_name in enumerate(extra_f_names):
            features_extra[:, idx] = np.asarray(plydata.elements[0][attr_name])
        # Reshape (P,F*SH_coeffs) to (P, F, SH_coeffs except DC)
        features_extra = features_extra.reshape((features_extra.shape[0], 3, (self.max_sh_degree + 1) ** 2 - 1))*0
