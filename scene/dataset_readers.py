...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, masks_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        mask_path = os.path.join(masks_folder, image_name) if masks_folder != "" else ""
        if mask_path!="":
            mask = cv2.imread(glob.glob(f"{mask_path}.*")[0]).astype(np.float32)
        else:
            mask = None
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png"):
    ...
    # train_cam_infos = train_cam_infos[:6]
